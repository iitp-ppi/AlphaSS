...
# PSH modifying for taking disulf_disto
def disulf_disto(protein):
    protein["disulf_disto"] = protein["disulf_disto"].clone().detach().to(torch.float32)
...
def random_crop_to_size(
 ...
 # PSH modifying.. channge the random crop method
def crop_to_size_disulf_info(
    protein,
    crop_size,
    site=None,
):
    """Crop randomly to `crop_size`, or keep as is if shorter than that."""
    seq_length = protein["seq_length"][0]
    
    
    if int(seq_length) <= crop_size: 
        return protein
    
    # in this case, the sequence length is longer than crop size.
    num_res_crop_size = seq_length - site
    ...
    for k, v in protein.items():
        ...
        criteria = seq_length - site
        
        if criteria <= crop_size:
            length_criteria = criteria
            if len(v) == seq_length:
                if len(v.shape) == 1:
                    v = v[site:-1]
                if len(v.shape) == 2:
                    if len(v[0]) == seq_length:
                        v=v[site:-1,site:-1]
                    else:
                        v = v[site:-1, :]
                if len(v.shape) == 3:
            ...
            if len(v) == seq_length:
                ...
                if len(v.shape) == 2:
                    if len(v[0]) == seq_length:
                        v=v[site:site+384,site:site+384]
                    else:
                        v = v[site:site+384, :]
                if len(v.shape) == 3:
                    if len(v[0]) == seq_length:
    ...
    np.place(protein["seq_length"], protein["seq_length"]==seq_length, length_criteria)
