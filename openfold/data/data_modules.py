...
class OpenFoldSingleDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        if feature_dir.split('.')[-1] == 'json':
            with open(feature_dir, 'rb') as jf:
                f_dict = json.load(jf)
            feature_dir_list = list(f_dict.keys())
            final_feature_list = list()
            for f_path in feature_dir_list:
                total_feature = f_dict[f_path]
                if len(total_feature) == 0:
                    pkl_name = f_path.split('.')[0]
                    final_feature_list.append(pkl_name)
                    continue
                else:
                    for size in range(len(total_feature)):
                        pkl_name = f_path.split('.')[0]
                        revise_name = pkl_name+'_'+str(size)
                        final_feature_list.append(revise_name)
        else:
            final_feature_list = list()
            feature_list = os.listdir(feature_dir)
            for f_path in feature_list:
                feature_path = feature_dir + f_path
                final_feature_list.append(feature_path)
        ...
        if mapping_path is not None:
            ...
            if feature_dir is not None:
                self.mapping = {
                    ...
                    for i, name in enumerate(final_feature_list)  #if i < 10 # or mode == 'eval'
                }
                
                self.reverse_mapping = { v:k for k,v in self.mapping.items() }
                
            elif(feature_dir is None):
                
                temp_mapping = {}
                i = 0
                for name in os.listdir(alignment_dir):
                    if len(name) <= 10:
                        temp_mapping[str(i)] = os.path.splitext(name)[0]
                        i += 1
    ...
    def __getitem__(self, idx):
        ...
        check = list(target.split('_'))
        if len(check) == 2:
            file_name = name
            site_num = None
        else:
            file_name, site_num = name.rsplit('_', 1)
        

        if(self.feature_dir is None):
            alignment_dir = os.path.join(self.alignment_dir, file_name)
            if(self.mode == 'train' or self.mode == 'eval'):
                ...
                if(os.path.exists(path + ".cif")):
                    ...
                    raise ValueError(f"{spl} : Invalid file type")
            else:
            ...
            data = pickle.load(open('%s.pkl' % (file_name),'rb'))
            if self.mode == 'train':
                msa = data['msa']
            ...
            if site_num != None:
                site = data['crop_site'][int(site_num)]
                data = data_transforms.crop_to_size_disulf_info(
                    protein=data,
                    crop_size = 384,
                    site=site
                )
...
class OpenFoldDataModule(pl.LightningDataModule):
    def __init__(self,
        ...
        **kwargs
        ...
        elif(val_data_dir is not None and val_alignment_dir is None and self.val_feature_dir is None):
            raise ValueError(
