...
def distogram_loss(
    ...
    **kwargs,
    ...
    #print("square_mask.shape, shape_mask.sum()", square_mask.shape, square_mask.sum())
...
def compute_tm(
    ...
    **kwargs,
    ...
    if len((weighted == torch.max(weighted)).nonzero()) == 0 :
...
class AlphaFoldLoss(nn.Module):
    ...
    def __init__(self, config, SummaryWriter = None):
        ...
        # add batch idx.. for tensorboard logging.... 
    def forward(self, out, batch, batch_idx):
        ...
        for loss_name, loss_fn in loss_fns.items():
            ...
            if weight:
                ...
                if self.SummaryWriter is not None:
                    self.SummaryWriter.add_scalar('loss/' + loss_name, loss, global_step = batch_idx)
                cum_loss = cum_loss + weight * loss
                
        if self.SummaryWriter is not None:
            self.SummaryWriter.add_scalar('loss/cum_loss', cum_loss, global_step = batch_idx)
